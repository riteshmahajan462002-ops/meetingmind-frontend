...
export default function Navbar() {
  ...
  const navItems = ["Features", "How It Works", "Pricing"] as const;
...
shadow-glow hover:-translate-y-1 transition-all duration-300">
            Get Started
          </button>
        </div>
        ...
        <div className="md:hidden flex flex-col pt-4 pb-6 border-t border-border-subtle gap-1 bg-bg-primary/95 backdrop-blur-xl">
          {navItems.map((item) => (
            <Link
              ...
              href="#"
              onClick={closeMenu}
              className="block text-base font-medium px-4 py-3 rounded-xl text-text-secondary hover:text-text-primary hover:bg-accent-glow/50 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-accent-primary"
              aria-label={`Navigate to ${item}`}
            >
              {item}
            </Link>
          ...
          <div className="px-4 pt-3">
            <button className="w-full text-sm font-semibold py-3 rounded-xl bg-gradient-to-r from-accent-primary to-accent-secondary text-white shadow-xl hover:shadow-2xl transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-accent-glow/50">
              Get Started Free
            </button>
          </div>
        </div>
